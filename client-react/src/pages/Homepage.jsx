...
<motion.section 
  className="py-20 bg-gradient-to-r from-blue-300 to-white text-white relative overflow-hidden"
  initial={{ opacity: 0 }}
...
>
  ...
  <div className="max-w-4xl  h-75 mx-auto px-6 text-center relative z-10">
    <motion.h2
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, delay: 0.2 }}
      viewport={{ once: true }}
      className="text-4xl font-bold m-6"
    >
    ...
    >
      Join {Math.floor(Math.random() * 5000) + 1000}+ happy customers today
    </motion.p> */}
    ...
    >
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={() => document.getElementById('services').scrollIntoView({ behavior: 'smooth' })}
        className="bg-white text-blue-600 font-bold px-8 py-4 rounded-full transition-all duration-300 hover:shadow-lg flex items-center gap-2"
      >
      ...
      <motion.button
        ...
        className="border-2 border-white text-white font-bold px-8 py-4 rounded-full transition-all duration-300 hover:bg-white/10"
      >
        Contact Us
      </motion.button>
    </motion.div>
